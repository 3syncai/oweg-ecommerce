...
const ProductGallery = ({
  ...
  const clampLens = (clientX: number, clientY: number) => {
    ...
    const x = Math.max(
      half,
      Math.min(bounds.width - half, clientX - bounds.left),
    );
    const y = Math.max(
      half,
      Math.min(bounds.height - half, clientY - bounds.top),
    );
    return { x, y };
  };
  ...
  const updateLensFromPoint = (clientX: number, clientY: number) => {
    ...
    const relativeX =
      ((nextPos.x - LENS_SIZE / 2) / (bounds.width - LENS_SIZE)) * 100;
    const relativeY =
      ((nextPos.y - LENS_SIZE / 2) / (bounds.height - LENS_SIZE)) * 100;
    setZoomPreview({
      size: `${ZOOM_SCALE * 100}%`,
  ...
  const handleMainImageTouchEnd = (e: React.TouchEvent) => {
    ...
    const isTap =
      deltaTime < 300 && Math.abs(deltaX) < 10 && Math.abs(deltaY) < 10;
          ...
          >
            ...
            <div
              ...
              style={{
                ...
                borderRadius: "24px",
                boxShadow:
                  "0 0 0 1px rgba(255,255,255,0.4), 0 10px 35px rgba(0,0,0,0.2)",
                          ...
                          {productHighlights
                            ...
                            .map((highlight, idx) => (
                              <div key={idx} className="flex items-start gap-2">
                                ...
                                <span className="text-[10px] md:text-xs lg:text-sm text-gray-600 leading-relaxed">
                                  {highlight}
                                </span>
                              </div>
                            ))}
