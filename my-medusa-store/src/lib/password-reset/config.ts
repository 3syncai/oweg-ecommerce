...
const DEFAULT_RESET_ATTEMPT_WINDOW_MINUTES = 15
const DEFAULT_MAX_RESET_ATTEMPTS_PER_IP = 10
const DEFAULT_AUDIT_RETENTION_DAYS = 30
const DEFAULT_TOKEN_RETENTION_HOURS = 48
...
export const passwordResetConfig = {
  tokenBytes: 32,
  tokenTtlMinutes: parsePositiveInt(
    process.env.PASSWORD_RESET_TOKEN_TTL_MINUTES,
    DEFAULT_TOKEN_TTL_MINUTES,
    { min: 5, max: 120 }
  ),
  requestWindowMinutes: parsePositiveInt(
    process.env.PASSWORD_RESET_WINDOW_MINUTES,
    DEFAULT_REQUEST_WINDOW_MINUTES,
    { min: 5, max: 1440 }
  ),
  maxRequestsPerIp: parsePositiveInt(
    process.env.PASSWORD_RESET_MAX_REQUESTS_PER_IP,
    DEFAULT_MAX_REQUESTS_PER_IP,
    { min: 1, max: 20 }
  ),
  maxRequestsPerEmail: parsePositiveInt(
    process.env.PASSWORD_RESET_MAX_REQUESTS_PER_EMAIL,
    DEFAULT_MAX_REQUESTS_PER_EMAIL,
    { min: 1, max: 20 }
  ),
  resetAttemptWindowMinutes: parsePositiveInt(
    process.env.PASSWORD_RESET_ATTEMPT_WINDOW_MINUTES,
    DEFAULT_RESET_ATTEMPT_WINDOW_MINUTES,
    { min: 1, max: 1440 }
  ),
  maxResetAttemptsPerIp: parsePositiveInt(
    process.env.PASSWORD_RESET_MAX_RESET_ATTEMPTS_PER_IP,
    DEFAULT_MAX_RESET_ATTEMPTS_PER_IP,
    { min: 1, max: 100 }
  ),
  auditRetentionDays: parsePositiveInt(
    process.env.PASSWORD_RESET_AUDIT_RETENTION_DAYS,
    DEFAULT_AUDIT_RETENTION_DAYS,
    { min: 1, max: 365 }
  ),
  tokenRetentionHours: parsePositiveInt(
    process.env.PASSWORD_RESET_TOKEN_RETENTION_HOURS,
    DEFAULT_TOKEN_RETENTION_HOURS,
    { min: 1, max: 720 }
  ),
  requireVerifiedEmail: parseBoolean(
    process.env.PASSWORD_RESET_REQUIRE_VERIFIED_EMAIL,
    false
  ),
  frontendUrl: normalizeUrl(
    process.env.STOREFRONT_URL || process.env.NEXT_PUBLIC_APP_URL
  ),
  smtp: {
...
export function hasPasswordResetMailerConfig() {
  const { smtp } = passwordResetConfig
