// WORKING MIGRATION with LOGIN
// Run: node src/scripts/migrate-with-login.js --dry-run
//      node src/scripts/migrate-with-login.js

require('dotenv/config')
const mysql = require('mysql2/promise')

const MEDUSA_URL = 'http://localhost:9000'
const ADMIN_EMAIL = 'admin@oweg.com'
...
async function login() {
  const res = await fetch(`${MEDUSA_URL}/admin/auth/user/emailpass`, {
    method: 'POST',
...
async function main() {
  ...
  console.log(`\n🔄 Price Migration with Login (${isDryRun ? 'DRY RUN' : 'LIVE'})\n`)
  ...
  try {
    ...
    if (isDryRun) {
      ...
      console.log('💡 Refresh Medusa Admin - all 1,522 prices updated!\n')
    }
